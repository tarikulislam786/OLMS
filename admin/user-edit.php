...
if(isset($_GET["id"])) {
    ...
    if (!empty($userId)) {
        ...
        $stmt = $conn->prepare("SELECT users.id, first_name, last_name, email, phone,role, password, students.discipline_id as sdiscipId,teachers.discipline_id as tdiscipId, session_id,roll_no,designation FROM (users LEFT JOIN students ON users.id = students.user_id) LEFT JOIN teachers on users.id = teachers.user_id LEFT JOIN librarians on users.id = librarians.user_id WHERE users.id =".$userId);
        $stmt->execute();
        $row = $stmt->fetch();
        ...
        if(!empty($row['sdiscipId'])) // referred to as student discipline_id
        {
            $discipline = $row['sdiscipId'];
        }
        if(!empty($row['tdiscipId']))  // referred to as teacher discipline_id
        {
            $discipline = $row['tdiscipId'];
        }
